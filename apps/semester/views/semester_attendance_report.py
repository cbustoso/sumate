...
def generate_attendee_excel(request, semester_id):
    filename = generate_filename(semester_id)
    attendances = get_attendances(semester_id)
...
def get_attendances(semester_id):
    ...
    query = Attendance.objects.filter(
        ...
        'event__name', 'event__event_type__name', 'event__semester__name', 'created_at', 'attendee__career__name',
        'points'
    ).annotate(
        total_points=Subquery(points_sum_subquery)
    ).order_by('attendee__career__name')
